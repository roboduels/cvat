...
export function getJobAsync(tid: number, jid: number, initialFrame: number, initialFilters: object[]): ThunkAction {
    return async (dispatch: ActionCreator<Dispatch>): Promise<void> => {
        try {
            ...
            let chunkProgress = Math.ceil((100 * (task.dataChunkSize * (iterateChunk + 1))) / (job.stopFrame + 1));
            ...
            while (++iterateChunk * task.dataChunkSize <= job.stopFrame) {
                ...
                chunkProgress = Math.ceil((100 * (task.dataChunkSize * (iterateChunk + 1))) / (job.stopFrame + 1));
                dispatch(updateChunkProgress(chunkProgress));
            }
        } catch (error) {
            dispatch({
